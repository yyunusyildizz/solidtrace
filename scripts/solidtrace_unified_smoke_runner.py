...
def main() -> int:
    ...
    # 3) Story preview
    story = find_script("story_preview_smoke_test.py")
    if story:
        run_cmd(
            "Story preview smoke",
            [
    ...
    # 5) Case preview
    case_preview = find_script("case_preview_smoke_test.py")
    if case_preview:
        run_cmd(
            "Case preview smoke",
            [
        ...
        print("⚠️ case_preview_smoke_test.py bulunamadı, atlandı.")

    # 6) Optional response action: unisolate
    if args.response:
    ...
    # 8) Optional synthetic generator
