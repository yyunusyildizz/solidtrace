...
from app.core.security import get_current_user, verify_agent_request
...
async def process_single_event(event_data: dict, tenant_id: Optional[str] = None):
    ...
    try:
        ...
        final_details = raw_details.lstrip("\ufeff").strip()
        ...
        full_text = f"{final_details} {event_data.get('command_line')} {event_data.get('type')}".lower()
        ...
        alert = AlertModel(
            id=str(uuid.uuid4()),
            created_at=event_data.get("timestamp") or datetime.now().isoformat(),
        ...
        logger.info(f"📥 1 event işlendi | host={event_data.get('hostname')} | type={event_data.get('type')}")
    except Exception as e:
        db.rollback()
        logger.error(f"Queue event işleme hatası: {e}")
        raise
    finally:
        db.close()
...
async def kill_process(req: ActionRequest, current_user: str = Depends(get_current_user)):
    ...
    db = SessionLocal()
    try:
        await write_audit(db, current_user, "KILL_PROCESS", target=f"{req.hostname}:PID{req.pid}", detail=req.rule or "")
    finally:
        db.close()
